...
msg_by_foo = "the message by foo"
msg_by_bar = "the message by bar"
...
def test_timelines():
    ...
    _, http_session = register_and_login("foo", "default")
    add_message(http_session, msg_by_foo)
    logout(http_session)
    _, http_session = register_and_login("bar", "default")
    add_message(http_session, msg_by_bar)
    r = http_session.get(f"{BASE_URL}/public")
    assert msg_by_foo in r.text
    assert msg_by_foo in r.text

    # bar's timeline should just show bar's message
    r = http_session.get(f"{BASE_URL}/")
    assert msg_by_foo not in r.text
    assert msg_by_bar in r.text
    ...
    assert msg_by_foo in r.text
    assert msg_by_bar not in r.text
